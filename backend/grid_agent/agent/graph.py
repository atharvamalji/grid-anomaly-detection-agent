...
from grid_agent.weather import OpenMeteoClient

from . import nodes
from .state import AgentState


def build_graph(llm: Provider, rag_index: RagIndex, weather_client: OpenMeteoClient):
    """Build the anomaly-explanation graph:
    classify -> fetch_weather -> retrieve -> explain -> recommend -> cite
    """
    graph = StateGraph(AgentState)

    graph.add_node("classify", lambda state: nodes.classify_anomaly(state, llm))
    graph.add_node("fetch_weather", lambda state: nodes.fetch_weather(state, weather_client))
...
def analyze_anomaly(
    ...
    app = build_graph(llm, rag_index, weather_client)
